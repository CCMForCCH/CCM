...
public class TrashSchedule {
	...
	public TrashSchedule(List<TrashPerson> trashPersonListOrig, List<TrashCycleRow> trashCycleDBRows)  {
	...
	public List<TrashRow> getTrashRows() {
		...
		// Set these for first time thru; assume there is at least one entry in the table
		this.newCycleStartingDate = trashCycleDBRows.get(0).getTrashcyclestartdate();
		this.newNextUseridToSkip = trashCycleDBRows.get(0).getNextuseridtoskip();
		...
		//for (int cycleIdx = 0; cycleIdx < 36; cycleIdx++) {
			...
			if (trashCycleDBRows.size() > cycleIdx) {
				...
				localCycleStartingDate = trashCycleDBRows.get(cycleIdx).getTrashcyclestartdate();
				localNextUseridToSkip = trashCycleDBRows.get(cycleIdx).getNextuseridtoskip();
			} else {
			...
			// (Note: this method sets the nextUseridToSkip value for the next cycle.)
			trashPersonList = 
					removeSkippedPeopleFromList(trashPersonListOrig, localNextUseridToSkip);
		
			// Get all the printable rows, unformatted
			TrashCycle trashCycle = new TrashCycle(trashPersonList, localCycleStartingDate);
		...
		for (TrashTeam oneTeam : trashTeams) {
			...
			if (calTeamDate.get(Calendar.YEAR) == calDateNow.get(Calendar.YEAR) &&
				calDateNow.get(Calendar.DAY_OF_YEAR) > calTeamDate.get(Calendar.DAY_OF_YEAR)) {
				rowsRemovedFromFirstCycle++;
				continue;
			}			
		...
		// OK, now remove entries from the last cycle based on those removed from
		...
		for (int tpIdx = 0; tpIdx < trashPersonListOrig.size(); tpIdx++) {
			if (skippingMode) {
				if (peopleToSkip != 0) {
					...
					skippingMode = false;
					this.newNextUseridToSkip = trashPersonListOrig.get(tpIdx).getUsername();
				}
			}
			if (peopleToSkip != 0) {
